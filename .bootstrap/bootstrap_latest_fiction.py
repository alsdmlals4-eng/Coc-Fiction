...
import base64, hashlib, io, json, re, shutil, sys, tarfile
...
while i < len(lines):
    ...
    if j >= len(lines) or lines[j].strip() != "---":
        raise SystemExit(f"chapter {number}: missing POV separator")
    j += 1
    while j < len(lines) and not lines[j].strip():
        j += 1
    if j >= len(lines):
        raise SystemExit(f"chapter {number}: missing POV")
    pov = lines[j].strip()
    j += 1
    body_start = j
    while j < len(lines) and not end_re.match(lines[j].strip()):
        j += 1
    if j >= len(lines):
        raise SystemExit(f"chapter {number}: missing end marker")
    end_match = end_re.match(lines[j].strip())
    if int(end_match.group(1)) != number:
        raise SystemExit(f"chapter {number}: mismatched end marker")
    body = "\n".join(lines[body_start:j]).strip()
    if number in chapters:
        raise SystemExit(f"chapter {number}: duplicate")
    chapters[number] = (title, pov, body, source_start, j + 1)
    i = j + 1

if set(chapters) != set(range(1, 226)):
    missing = sorted(set(range(1, 226)) - set(chapters))
    extra = sorted(set(chapters) - set(range(1, 226)))
...
for number, (_, _, body, _, _) in chapters.items():
    if len(body) < 2000:
        raise SystemExit(f"chapter {number}: below 2,000 chars ({len(body)})")
    stale = [term for term in forbidden if term in body]
    if stale:
        raise SystemExit(f"chapter {number}: stale active terms {stale}")

def group(number: int) -> tuple[str, str]:
    if number <= 70:
        return "part-1", "1부 《폭풍의 눈》"
    if number <= 95:
        return "side-story-lake", "외전1 《호수가 보이는 마을》"
    if number <= 130:
        return "side-story-alice", "외전2 《붉은 여왕의 첫 임무》"
    if number <= 165:
        return "side-story-ian", "외전3 《미래를 기록하는 사람》"
...
index = {
    "schema_version": 1,
    "source": "latest integrated Google Doc export used as migration evidence",
    "generated_at": "2026-07-22",
    "counting_rule": "본문만 집계하며 시작·끝 마커와 POV 표식은 제외한다. 공백과 줄바꿈은 포함한다.",
    "chapters": [],
}
for lower in range(1, 226, 5):
    upper = min(lower + 4, 225)
    directory, label = group(lower)
    bundle = f"fiction/manuscript/{directory}/{lower:03d}-{upper:03d}.md"
    output = [
        f"# {label} · 제{lower}화~제{upper}화",
        "",
        "> 상태: 2,000자 이상 확장 원고 DRAFT. 전체 구조·연속성·문장 심화 퇴고 전.",
        "",
    ]
    for number in range(lower, upper + 1):
        title, pov, body, source_start, source_end = chapters[number]
        output.extend([
            f"## 제{number}화 · {title}",
            "",
            f"**POV:** {pov}",
...
(ROOT / "fiction/MANUSCRIPT_INDEX.json").write_text(
...
p1 = ROOT / "fiction/bible/03_PART1_STORY_BIBLE_SOURCE.md"
p1.write_text(
    "# 1부 상세 기획 원문 변환본\n\n"
...
lengths = [item["body_chars"] for item in index["chapters"]]
print(f"generated 225 chapters, 45 bundles, min={min(lengths)}, max={max(lengths)}, avg={sum(lengths)/len(lengths):.2f}")
