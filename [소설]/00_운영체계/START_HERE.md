...
현재 작품은 `REVISE`다. 225화 모두 2,000자 이상 확장됐지만 구조·연속성·문체·최종 교정은 완료되지 않았다.

## 프로젝트 Skill

- `fiction-project-operations`: 범위·동시 작업·체크포인트·인수인계
- `fiction-story-development`: 코어·주제·인물·인과·대표 장 품질
- `fiction-drafting`: POV·대화·묘사·행동·리듬 집필
...
최신 사용자 지시 → `fiction/bible/01_PROJECT_CORE.md` → `fiction/CANON_REGISTRY.json` → 부별 바이블·연속성 → 현재 원고 → 외부 참고 순이다.
