...
현재 대화의 제1~105화 통합 재퇴고본은 최신 작업 증거이지만 GitHub `manuscript/`로 전파됐다는 증거가 없으므로 자동 정본 승격하지 않는다. `ACTIVE_CONTEXT.md`의 reconciliation 절차에 따라 원본·최신 사용자 지시·현행 Canon과 대조해 선택적으로 반영한다.
...
- 현재 GitHub 225화는 모두 하한을 충족한 확장 원고 DRAFT다. 하한 충족은 전체 퇴고 완료가 아니다.
...
현재 운영 우선순위는 `ACTIVE_CONTEXT.md`와 `analysis/SCENE_PASS_REGISTRY.json`이 책임진다. 2026-08-10 기준 다음 묶음은 **제1~105 외부 통합본 정본 reconciliation의 첫 단위 `001-005`**이며, 기존 `176-180` 원본 직접 대조는 이후 대기 작업으로 보존한다.
