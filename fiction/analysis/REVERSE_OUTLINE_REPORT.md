...
이 보고서는 **mixed migration 상태의 GitHub 저장 원고**를 설명한다. `001–020`은 current candidate와 대조된 production prefix이지만, 제21화 이후 stored chapter는 자신의 reconciliation을 통과하기 전까지 최신 external narrative와의 연속성을 주장할 수 없다.
...
- Ch5→6, Ch10→11, Ch15→16은 current continuity로 정상 연결한다.
...
3. Ch20 종료 상태와 current Ch21의 배수로 도주 연결을 먼저 확인한다.
