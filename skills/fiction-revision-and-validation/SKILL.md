...
장편 연재의 묶음 단위 퇴고·원본 대조에는 `serial-arc-pass`를 사용한다. 이 mode는 새 장르 공식이나 별도 Canon을 만들지 않고, 현재 책임 원본과 같은 기준 커밋의 원고·장면 카드·색인·역개요·Registry·기획 문서를 한 변경 단위로 묶는다.

1. 현재 작업 계약과 `ACTIVE_CONTEXT`에서 대상 묶음, 직전·직후 경계 화, 원본 범위, 보호 대상, 제외 항목을 확정한다.
2. 원본 인벤토리·Canon·부별 바이블을 확인한 뒤 시간, 위치, POV, 지식, 부상, 소지품, 관계, 비밀, 기관 권한 상태를 경계까지 비교한다.
3. 묶음이 부별 약속·주제·인물 선택·복선·다음 압력에 수행하는 기능을 한 문장으로 판정하고, 대표 품질 게이트와 비교하되 한 묶음의 결함을 전체 원고 결함으로 일반화하지 않는다.
4. finding을 `MUST_FIX / SHOULD_FIX / DEFER / REJECT / UNVERIFIED`로 재판정하고 승인 범위의 최소 수정만 수행한다.
5. 원고가 바뀌면 `MANUSCRIPT_INDEX`, 역개요 override, Scene Pass Registry, Scene Card, Revision Report, `ACTIVE_CONTEXT`, `HANDOFF`, 현행 기획서를 같은 변경 범위에서 재생성·갱신한다.
6. 자동 역개요·분량 통계·패턴 검사는 finding 후보일 뿐 수정 명령이 아니다. 원고 문맥·Canon·직접 대조 근거로 재판정한다.
7. 완료 뒤 묶음 내부와 경계, 부 단위 영향, 225화 소비자, 구형 참조, 변경하지 않은 원고 SHA를 회귀 검사한다.

결과는 `작업 계약 / 원본·Canon 근거 / 묶음 기능 / 경계 연속성 / finding 판정 / 최소 수정 / Canon 전파 / 파생 자료·기획 문서 갱신 / 보존·미검증 범위 / 다음 묶음` 순서로 보고한다.
