...
class Promote036040ContractTests(unittest.TestCase):
    ...
    def test_historical_036_040_promotion_receipt_is_preserved(self):
        ...
        # A later valid bounded promotion may move the live frontier past Ch40.
        self.assertGreaterEqual(rec["reconciled_prefix_end"], 40)
        ...
        self.assertEqual(
            item["preserved_boundary_shas"]["35"],
            "c4b02af3eb326dfd18ec0331c762c92655cb97525b8b3223d407e69ce912d5f2",
        )
        self.assertEqual(
            item["chapter_shas"],
            {str(number): EXPECTED[number]["sha"] for number in range(36, 41)},
        )
        sources = [entry for entry in item.get("source_files", []) if isinstance(entry, dict)]
        self.assertTrue(
            any(
                entry.get("name") == SOURCE_FILE and entry.get("sha256") == SOURCE_SHA256
    ...
    def test_reverse_outline_keeps_promoted_036_040_connected_after_later_promotions(self):
        ...
        if frontier > 40:
            self.assertEqual(by_chapter[40]["next_chapter"]["chapter"], 41)
            self.assertNotIn("RECONCILIATION_MIGRATION_BOUNDARY", by_chapter[40]["structural_flags"])
            self.assertEqual(by_chapter[41]["previous_chapter"]["chapter"], 40)
            self.assertNotIn("LEGACY_TAIL_BOUNDARY", by_chapter[41]["structural_flags"])
        else:
