...
expected_passes = {
    ...
    "fiction/manuscript/part-1/006-010.md": {
        "chapters": [6, 7, 8, 9, 10],
        "boundaries": [5, 11],
        "card_boundaries": ["제5→6화", "제6→7화", "제7→8화", "제8→9화", "제9→10화", "제10→legacy-tail"],
    },
...
required_phrases = {
    ...
    5: "신호기 잃어버리지 마세요",
    6: "그 기계는 받기만 한다.",
    7: "그리고 더 깊은 곳에서 익숙한 여자의 목소리가 들렸다. “선배?”",
    8: "같은 편이 아니긴 하네요.",
    9: "친구가 안전하다는 판단은.",
    10: "그리고 그 손실은 책보다 무거웠다.",
    91: "답을 찾았느냐",
...
forbidden_phrases = {
    6: ["복도에서 첫 총성이 울리기 전"],
    7: ["자기 안의 차가운 보호 반응", "다른 자신이 문을 세게 두드렸다"],
...
if reconciliation.get("reconciled_prefix_end") != 10:
    errors.append("reconciled prefix must end at chapter 10 after the second pass")
if reconciliation.get("legacy_tail_starts_at") != 11:
    errors.append("legacy tail must begin at chapter 11 after the second pass")
if reconciliation.get("boundary_after_chapter") != 10:
    errors.append("migration boundary must be declared after chapter 10")
...
if chapter10_outline.get("next_chapter") is not None:
    errors.append("chapter 10 reverse outline must not claim a legacy-tail next chapter")
...
if "정본 마이그레이션 경계" not in chapter10_outline.get("evidence", {}).get("next_pressure", ""):
    errors.append("chapter 10 reverse outline missing migration-boundary pressure")

chapter11_outline = outline_entries.get(11, {})
if chapter11_outline.get("previous_chapter") is not None:
    errors.append("legacy chapter 11 reverse outline must not claim reconciled chapter 10 as previous continuity")
...
print(
    ...
    "(001-010 external reconciled; migration boundary moved to 10→11; "
